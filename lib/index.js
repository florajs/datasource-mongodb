...
class DataSource {
        ...
        async.waterfall([
            ...
            (docs, next) => {
                ...
                collection.find(query, options).count()
                    .then(count => next(null, { totalCount: count, data: docs }));
            }
        ], callback);
    }
     ...
     */
    close(callback) {
        async.parallel(Object.keys(this.connections).map(connId => (next) => {
    ...
    getConnection(database, callback) {
        if (this.connections[database]) {
            return callback(null, this.connections[database].db(database));
        }
        ...
        return mongodb.MongoClient.connect(url, { useNewUrlParser: true }, (err, client) => {
            if (err) return callback(err, null);
            this.connections[database] = client;
            return callback(null, client.db(database));
        });
    }
}
