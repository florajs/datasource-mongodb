...
DataSource.prototype.process = function (request, callback) {
    ...
    this.getConnection(request.database, function (err, db) {
        ...
        db.collection(request.collection, function onCollection(err2, collection) {
            if (err2) return callback(err2, null);
            collection.find(query, options).toArray(function (err3, docs) {
...
DataSource.prototype.close = function (callback) {
    ...
    var closeFn = Object.keys(this.connections).map(function (connId) {
        return new Promise(function (resolve) {
            self.log.trace('closing MongoDB connection "' + connId + '"');
            self.connections[connId].close(resolve);
        });
    });

    Promise.all(closeFn).then(function () {
