        ...
        andFilters.forEach((condition) => {
            if (condition.operator === 'equal') {
                ...
                throw new ImplementationError(
                    'Filter operator "' + condition.operator + '" not implemented in data-source "mongodb"'
                );
            }
        });
...
class DataSource {
    ...
    constructor(api, options) {
        this.log = api.log.child({ component: 'datasource-mongodb' });
        this.connections = {};
        this._closing = false;
    ...
    async process(request) {
        ...
        const totalCount = await collection.find(query, options).count();
    ...
    close() {
        ...
        return Promise.all(
            Object.keys(this.connections).map((connId) => {
                ...
                return this.connections[connId].then((client) => client.close());
            })
        );
    }
    ...
    getConnection(database) {
        if (this._closing) return Promise.reject(new Error('Instance is closing'));
        ...
        if (!this.connections[database]) {
            ...
            const client = mongodb.MongoClient.connect(url, {
        ...
        return this.connections[database].then((client) => client.db(database));
    }
}
